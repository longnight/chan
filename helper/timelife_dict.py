# -*- coding: utf-8 -*-
# This container stores its items both in a dict (for direct access) and
# in a bi-directionally linked list, which enables it to update itself
# in essentially O(1) time. No iteration over the entire list is ever needed,
# no separate thread is required for cleaning either.
# Should be useful e.g. for session storage in web servers.
# http://code.activestate.com/recipes/498266-a-self-cleaning-dict-like-container-which-limits-t/
...
class _FakeLock(object):
    '''
    a do-nothin, substituted for a real Lock if there is no threading.
...
def RLock():
    ...
    '''
    # thread may be imported either directly or by module threading
...
class _Item(object):
    '''
    wrapper for items stored in LruDict,
    providing them with references to one another
    '''
...
class LruDict(DictMixin):
    ...
    def _setNewest(self, item):
        ...
        if item is self._newest:
            return

        # this item is currently in the pile...
        if item.nextItem or item.previousItem:
            self._pullout(item)                         # pull it out

        if self._newest:
            # point the previously newest item to this one...
            self._newest.nextItem = item
            item.previousItem = self._newest            # and vice versa

        # reset the 'newest' pointer
        self._newest = item

        # this only applies if the pile was empty
        if not self._oldest:
            self._oldest = item
    ...
    def _pullout(self, item):
        ...
        # we are somewhere in between at least 2 others -
        # hitch up the neighbours to each other
        else:
    ...
    def __setitem__(self, key, value):
        ...
        try:
            ...
            # if we prune a the beginning, we may wind up with size+1
            # items; if we prune at the end,
            # we might keep an expired item. Not serious.
            self.prune()
    ...
    def prune(self):
        ...
        try:
            ...
            while (len(self._data) > self._size or self._oldest and
                    self._oldest.atime < outtime):
    ...
    def __repr__(self):
        d = dict(self.items())
        return '%s(timeout=%s, size=%s, data=%s)' % (
            self.__class__.__name__, self._timeout, self._size, repr(d))
...
if __name__ == '__main__':
    ...
    size = 10000
    items = 100000
    print '\ncreating a LruDict with length %d and setting %d items' % (
        size, items)
